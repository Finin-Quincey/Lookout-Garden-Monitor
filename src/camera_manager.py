...
class VideoStream:
    """
    Camera object that controls video streaming from the camera. This uses a separate thread to the object detection code
    so that the capturing can happen continuously while the object detection just reads the latest frame from here whenever
    it starts a new detection cycle (usually about 2 times per second).
    
    I moved the video display code in here as well so that the video runs at the capture framerate and the object detector
    annotations update each time the detector completes a cycle.
    
    Original Source - Adrian Rosebrock, PyImageSearch:
