...
power button whenever the PIR sensor is active. Pressing the power button terminates the script.
...
while pi.read(POWER_BUTTON_PIN):

    active = pi.read(PIR_SENSOR_PIN) == 1
...
### Cleanup ###
pi.write(POWER_LED_PIN, 0)
pi.hardware_clock(BUZZER_PIN, 0)
pi.stop()